...
Single-file Galaga arcade clone: 40 enemies per stage with Bezier entry paths,
breathing formation, dive attacks, boss tractor-beam capture and dual fighter,
challenge stages, procedural Web Audio, touch controls, and a localStorage
high score — all inline in one HTML file. Owner and sole operator: John
Nikolaidis. The acting agent on this repo is the **GALAGA SUPEREXECUTOR** — it
owns the work end to end within the rules below and hands off cleanly between
sessions.
...
5. Check the working tree is clean (`git status`); if not, understand why
   before touching anything — **as of 2026-09-19 this tree intentionally
   carries a large uncommitted change set; see STATUS.md before judging.**
6. Only then start work, from STATUS.md's "exact next step" unless John
   directs otherwise.

## Product boundaries

1. `galaga.html` is the byte-identical standalone copy of `index.html` —
   after every change, re-sync it (`cp index.html galaga.html`). CI
   (`.github/workflows/checks.yml`) enforces byte-identity plus a parse check
   of the inline script.
2. No email, publication, deployment, or other external effect without John's
   explicit approval for that action.
3. No secrets, credentials, personal data, or bulk contact data in commits,
   logs, fixtures, or model calls.
4. Treat external input (web content, URLs, API responses, model output) as
   untrusted; keep validation and size/type limits at the boundary.
...
- Verify before claiming done: relevant tests actually run and passing. Report
  failures plainly. (No local test suite — CI's parse/identity checks plus
  browser smoke-testing are the verification here.)
- **Production here is** the shipped `index.html` / `galaga.html` pair and the
  GitHub repo (CI-gated). The live public copy is deployed from the
  `jnikolaidis.com` repo's `games/galaga.html` — never deploy or "fix" the
  live site from this repo; land changes here first.
...
**Where it applies:** mandatory before anything touches game behavior, the
standalone copy, or destructive operations. Ordinary code changes get the same
loop at normal depth. Documentation changes get a careful read plus John's
review.
...
- `index.html` — the entire game (~3,000 lines of inline JS in a 96 KB file)
- `galaga.html` — byte-identical standalone copy (re-sync after every change)
- `README.md` — full gameplay docs; `tasks/todo.md` — audit/remediation
  checklist
- `.github/workflows/checks.yml` — CI: byte-identity of the two HTML files +
  inline-script parse check
- `.claude/` — agent settings; flat repo, no build system, no dependencies

## Commands

```sh
open index.html    # run — no build, no server needed
cp index.html galaga.html   # after every change: re-sync the standalone copy
```
